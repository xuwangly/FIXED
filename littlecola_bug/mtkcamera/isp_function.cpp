...
#define ISP_FUNC_DBG(fmt, arg...)        do { if (1  ) { BASE_LOG_DBG(fmt, ##arg); } } while(0)
...
dequeueHwBuf( MUINT32 dmaChannel, stISP_FILLED_BUF_LIST& bufList )
{
    ...
    if ( ISP_PASS1   == this->path || \
        ...
        if ( _imgo_ == rt_dma || \
			...
			int i =0;
			for(i = 0 ; i < 4 ; i ++)
				ISP_QUE_DBG(rt_dma,"[Movidius buf_ctrl] %d\n" ,((unsigned char *)(deque_buf.data[0].base_vAddr))[i]);
			#endif
			ISP_QUE_DBG(rt_dma,"[MovidiusTemp]: %d\n" ,((unsigned char *)(deque_buf.data[0].base_vAddr))[1]*100 
				+ ((unsigned char *)(deque_buf.data[0].base_vAddr))[2]);
            P1DeQCnt[rt_dma] -= 1;
            if( (_imgo_ == rt_dma) || (_rrzo_ == rt_dma) ){
                ISP_QUE_DBG(rt_dma,"imgo/rrzo: vf=0x%x, fbc: 0x%x/0x%x_0x%x/0x%x\n",vf,fbc[0],fbc[1],readReg(0x000040f0),readReg(0x000040f4));
            }
