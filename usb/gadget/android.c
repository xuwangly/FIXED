...
extern struct usb_function_instance *loopback_alloc_instance(void);
extern struct usb_function *loopback_alloc(struct usb_function_instance *fi);
...
static struct android_usb_function *supported_functions[] = {
	...
	[ANDROID_LOOPBACK] = &loopback_function,
	NULL
};
				   ...
				   char *name)
{
	...
	struct android_usb_function *f;
	struct android_usb_function_holder *f_holder;
	pr_err("[LOL]android_enable_function: begin enable %s function\n", name);
	while ((f = *functions++)) {

		pr_err("[LOL]android_enable_function:strcmp %s with %s\n", f->name ,name);
...
static int android_probe(struct platform_device *pdev)
{
	...
	if (len > ANDROID_MAX_FUNC_CNT) {
		...
		supported_list = devm_kzalloc(
				&pdev->dev, sizeof(supported_list) * (len + 1 + 1),
				GFP_KERNEL);
		if (!supported_list)
			return -ENOMEM;
		...
		supported_list[i] = supported_functions[ANDROID_LOOPBACK];
	}
	...
	android_dev->functions = 
		supported_list ? supported_functions : default_functions;
	pr_err("[LOL]use supported_functions?:(%d)\n", supported_list ? 1 : 0);
